...
> [!NOTE]
> Eta 支持 ColorOS 与 HyperOS，覆盖 OPPO 系（OPPO / 一加 / 真我）和小米系设备。App 本体是核心工作台并承载完整 Agent Runtime；在 ColorOS 上可直接成为默认数字助理，小布与小爱适配则作为兼容入口，共用 BYOK 自定义模型配置。完整功能需要 Root 和 LSPosed。
...
Eta 可在 Android 系统设置中直接注册为默认数字助理。ColorOS 长按电源键可在小布助手、Gemini 和 Eta 之间选择：小布模式保留系统原始行为，Gemini 模式完整沿用 Google 系统助手链路，Eta 模式打开自动聚焦输入框的文本会话。默认保持小布；旧版已经开启“长按电源键唤起 Gemini”的用户会继续使用 Gemini。默认助理自动设置是独立开关，只对 Gemini 和 Eta 生效。HyperOS 电源键入口尚未接入。

- **Eta 系统助理**：可作为电源键目标和 Android 默认数字助理；支持手动输入、流式回答、连续追问和结果归档，执行前台设备操作时自动收起文本面板并交给 Agent 操作浮层
...
5. 在系统设置中开启 Eta 无障碍服务；如需系统助手入口，在 Eta 设置页选择电源键目标，并按需开启默认助理自动设置
