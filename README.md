...
- **模型管理**：内置官方目录、远程拉取列表、自定义模型、启停管理，并可按展示名称或 Model ID 模糊搜索；各提供商分别记忆上次选择的模型。新增项仅在确认保存后入库，远程同步只更新远程来源，不删除手工模型。`/models` 明确返回的能力元数据优先，缺失时才由内置提供商的官方目录补齐；官方预设按提供商隔离，不把某个平台尚未提供的模型复制到它的目录中
